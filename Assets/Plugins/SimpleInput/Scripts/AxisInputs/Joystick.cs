...
namespace SimpleInputNamespace
{
	public class Joystick : MonoBehaviour, ISimpleInputDraggable
	{
		...
		private void Awake()
		{
			...
			if (innerImage == null && thumbTR.childCount > 0)
			{
				var child = thumbTR.GetChild(0);
				if (child != null) innerImage = child.GetComponent<Image>();
			}
		...
		private void Start()
		{
			...
			if (!isDynamicJoystick)
			{
				if (background) eventReceiver = background.gameObject.AddComponent<SimpleInputDragListener>();
				else eventReceiver = thumbTR.gameObject.AddComponent<SimpleInputDragListener>();
			}
			else
			{
		...
		public void OnPointerDown(PointerEventData eventData)
		{
			...
			if (innerImage != null && isDynamicJoystick)
			{
				innerImage.gameObject.SetActive(true);
				innerImage.raycastTarget = false;
			}
		...
		private void OnUpdate()
		{
			...
			if (innerImage != null)
			{
				Color c2 = innerImage.color;
				c2.a = opacity;
				innerImage.color = c2;

				bool shouldBeActive = opacity > 0.01f;
				if (innerImage.gameObject.activeSelf != shouldBeActive)
					innerImage.gameObject.SetActive(shouldBeActive);
			}
